"""
Gestão Fiscal SERVE — módulo Pagamentos por Conta 2026
Fluxo completo: login por gestor -> registo central de clientes (com interruptores
de que impostos se aplicam) -> dados e cálculo do PPC -> associar guias -> gerar
e enviar emails -> exportar.

Base de dados: Supabase (Postgres), com Row Level Security — cada gestor só vê e
edita os clientes que lhe estão atribuídos (campo Gestor_Email); o admin vê tudo.

Modelo de dados: "clientes" é o registo central, partilhado por toda a futura
plataforma (PPC, IVA, IMI, IRS, Segurança Social). Cada cliente tem interruptores
manuais (Aplica_PPC, Aplica_IVA, ...) que decidem em que módulos aparece. Os dados
específicos de cada imposto ficam em tabelas próprias — por agora só "ppc_dados".
...
def guardar_ppc_db(df: pd.DataFrame):
    """A RLS de ppc_dados segue a do cliente respetivo (junção por NIF), por
    isso este 'apagar tudo o que está visível + reinserir' já fica corretamente
    restrito à carteira de quem estiver a gravar."""
    client = get_client()
...
st.title("💶 Gestão Fiscal SERVE — Pagamentos por Conta 2026")
...
# --- TAB 1: Clientes (registo central) ----------------------------------
with tab1:
    st.subheader("Registo Central de Clientes")
    st.caption(
...
# --- TAB DASHBOARD ---------------------------------------------------------
with tab_dash:
    st.subheader("Visão Geral — PPC")
    total_clientes_registo = len(clean_clientes_df(st.session_state.clientes))
    st.caption(f"Registo central: {total_clientes_registo} cliente(s). Números abaixo referem-se apenas aos que têm 'Aplica PPC' ligado.")
    if df_calc.empty:
        st.info("Ainda não há clientes com 'Aplica PPC' ligado. Ativa esse interruptor na aba 'Clientes'.")
    else:
        ...
        st.caption("💾 Os dados ficam guardados de forma persistente no Supabase — não se perdem ao fechar o browser, exportar, ou voltar mais tarde. As guias em PDF carregadas na aba 'Guias', porém, só existem durante a sessão atual — o estado 'Guia Emitida' fica guardado, mas o ficheiro em si tens de recarregar se voltares noutro dia.")

# --- TAB 2: Dados e Cálculo do PPC ---------------------------------------
with tab2:
    ...
    if base_ppc.empty:
        ...
        editor_ppc = st.data_editor(
            base_ppc[PPC_COLS + ["Nome"]][["NIF", "Nome"] + [c for c in PPC_COLS if c != "NIF"]],
            use_container_width=True,
            hide_index=True,
