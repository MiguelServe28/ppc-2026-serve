...
TEXT_COLS = ["NIF", "Nome", "Email", "Idioma", "Gestor_Nome", "Gestor_Email", "Notas"]
...
DEFAULT_TEMPLATES = {
    1: {
        ...
        "EN": {
            "assunto": "2026 Corporate Tax Advance Payments — {nome}",
            "corpo": (
                "Dear Sir/Madam,\n\n"
                "Following the assessment of the 2025 corporate income tax (IRC), we inform you that {nome} "
                "(Tax ID {nif}) has advance tax payments due in 2026, with the following amounts and deadlines:\n\n"
                "• 1st Advance Payment: €{pag1} — due by {data1}\n"
                "• 2nd Advance Payment: €{pag2} — due by {data2}\n"
                "• 3rd Advance Payment: €{pag3} — due by {data3}\n\n"
                "Total for the year: €{total}\n\n"
                "Please find attached the payment slip for the 1st installment. We kindly ask that payment be made "
                "by the indicated deadline to avoid late payment interest.\n\n"
                "The payment slips for the 2nd and 3rd installments will be sent in due course.\n\n"
    ...
    2: {
        ...
        "EN": {
            "assunto": "2nd Advance Tax Payment 2026 — {nome}",
            "corpo": (
                "Dear Sir/Madam,\n\n"
                "Following the 1st advance payment already made, we remind you that the 2nd advance payment "
                "for {nome} (Tax ID {nif}) is due on {data2}, in the amount of €{pag2}.\n\n"
                "Please find the corresponding payment slip attached.\n\n"
                "Please do not hesitate to contact us should you have any questions.\n\n"
                "Best regards,"
            ),
        },
    ...
    3: {
        ...
        "EN": {
            "assunto": "3rd Advance Tax Payment 2026 — {nome}",
            "corpo": (
                "Dear Sir/Madam,\n\n"
                "Following the advance payments already made, we remind you that the 3rd and final advance payment "
                "for {nome} (Tax ID {nif}) is due on {data3}, in the amount of €{pag3}.\n\n"
                "Please find the corresponding payment slip attached.\n\n"
                "Please do not hesitate to contact us should you have any questions.\n\n"
                "Best regards,"
            ),
...
def init_db():
    ...
    conn.execute(f"""
        CREATE TABLE IF NOT EXISTS clientes (
            NIF TEXT PRIMARY KEY, Nome TEXT, Email TEXT, Idioma TEXT DEFAULT 'PT',
    ...
    """)
    # Migração: se a tabela já existia sem estas colunas, adiciona-as agora (sem apagar dados)
    ...
    if "Idioma" not in colunas_existentes:
        conn.execute("ALTER TABLE clientes ADD COLUMN Idioma TEXT DEFAULT 'PT'")
    conn.execute("""
...
# Estado
# ---------------------------------------------------------------------------
def migrar_templates_para_idiomas(templates: dict) -> dict:
    """Converte templates no formato antigo (sem PT/EN) para o novo formato aninhado,
    preservando o texto já personalizado como a versão PT e usando o EN por omissão."""
    if not templates:
        return copy.deepcopy(DEFAULT_TEMPLATES)
    migrado = {}
    for n in (1, 2, 3):
        entrada = templates.get(n) or templates.get(str(n))
        if entrada and "assunto" in entrada and "corpo" in entrada and "PT" not in entrada:
            # Formato antigo: {"assunto": ..., "corpo": ...} -> vira a versão PT
            migrado[n] = {
                "PT": {"assunto": entrada["assunto"], "corpo": entrada["corpo"]},
                "EN": copy.deepcopy(DEFAULT_TEMPLATES[n]["EN"]),
            }
        elif entrada and "PT" in entrada and "EN" in entrada:
            migrado[n] = entrada
        else:
            migrado[n] = copy.deepcopy(DEFAULT_TEMPLATES[n])
    return migrado
...
def init_state():
    ...
    if "params" not in st.session_state or "templates" not in st.session_state:
        ...
        if "templates" not in st.session_state:
            st.session_state.templates = migrar_templates_para_idiomas(templates_db)
            if templates_db != st.session_state.templates:
...
def clean_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Idioma"] = df["Idioma"].str.upper().apply(lambda v: v if v in IDIOMAS_VALIDOS else "PT")
    return df[CLIENT_COLS]
...
def gerar_excel(df_calc: pd.DataFrame, params: dict) -> bytes:
    ...
    headers = [
        "NIF", "Nome", "Email", "Idioma", "Gestor (nome)", "Gestor (email)", "Volume 2025", "Coleta 2025", "Retenções 2025",
    ...
    widths = [12, 26, 24, 9, 18, 24, 13, 13, 13, 13, 8, 12, 11, 11, 11, 11, 12, 12, 12, 11, 11, 11, 11, 11, 11, 20]
    ...
    for _, r in df_calc.iterrows():
        vals = [
            r["NIF"], r["Nome"], r["Email"], r["Idioma"], r["Gestor_Nome"], r["Gestor_Email"],
...
with tab1:
    ...
    with col1:
        up = st.file_uploader("Importar CSV ou Excel (colunas: NIF, Nome, Email, Idioma, Gestor_Nome, Gestor_Email, Volume_2025, Coleta_2025, Retencoes_2025)", type=["csv", "xlsx"])
        if up is not None:
            try:
    ...
    with col2:
        template_csv = pd.DataFrame(
            [{"NIF": "500123456", "Nome": "Empresa Exemplo, Lda.", "Email": "geral@exemplo.pt", "Idioma": "PT",
    ...
    st.caption("O 'Gestor' é opcional — se preenchido, o email desse gestor entra automaticamente em CC. O 'Idioma' determina se o cliente recebe o email em português ou inglês (por omissão: PT).")
    edited = st.data_editor(
        st.session_state.clientes,
        num_rows="dynamic",
        use_container_width=True,
        column_config={
            "Idioma": st.column_config.SelectboxColumn("Idioma", options=list(IDIOMAS_VALIDOS), default="PT"),
...
with tab4:
    ...
    if df_calc.empty:
        ...
        tpls_pagamento = st.session_state.templates[n_pag_email]  # {"PT": {...}, "EN": {...}}

        with st.expander("✏️ Editar templates deste email (PT e EN)"):
            idioma_edit = st.radio("Editar versão:", list(IDIOMAS_VALIDOS), horizontal=True, key=f"idioma_edit_{n_pag_email}")
            tpl_edit = tpls_pagamento[idioma_edit]
            tpl_edit["assunto"] = st.text_input("Assunto", value=tpl_edit["assunto"], key=f"assunto_{n_pag_email}_{idioma_edit}")
            tpl_edit["corpo"] = st.text_area("Corpo", value=tpl_edit["corpo"], height=300, key=f"corpo_{n_pag_email}_{idioma_edit}")
        ...
        if preview_nif:
            ...
            tpl_cliente = tpls_pagamento[row["Idioma"]]
            assunto, corpo = render_template(tpl_cliente, row, st.session_state.params)
            st.caption(f"🌐 Idioma deste cliente: {row['Idioma']}")
