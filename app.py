...
def init_state():
    if "clientes" not in st.session_state:
        st.session_state.clientes = pd.DataFrame(columns=CLIENT_COLS)
    if "guias" not in st.session_state:
        st.session_state.guias = {}  # {(nif, n_pagamento): (filename, bytes)}
    ...
    if "templates" not in st.session_state:
        st.session_state.templates = {k: v.copy() for k, v in DEFAULT_TEMPLATES.items()}
    if "log_envio" not in st.session_state:
        st.session_state.log_envio = []
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Fórmula: Total PPC = (Coleta IRC − Retenções) × Taxa, dividido em 3 prestações iguais (a 3.ª absorve o arredondamento). Dispensa se Total < limite definido.")

tab1, tab2, tab3, tab4, tab5 = st.tabs(
    ["📋 Clientes", "🧮 Cálculo PPC", "📎 Guias", "✉️ Emails", "⬇️ Exportar"]
)
...
with tab1:
            ...
            try:
                ...
                if st.button("Confirmar importação"):
                    if modo == "Substituir tudo":
                        st.session_state.clientes = novo
                    else:
                        st.session_state.clientes = clean_df(
                            pd.concat([st.session_state.clientes, novo], ignore_index=True)
                        ).drop_duplicates(subset="NIF", keep="last")
                    st.success(f"{len(novo)} clientes importados.")
                    st.rerun()
    ...
    if st.button("💾 Guardar alterações à tabela"):
        st.session_state.clientes = clean_df(edited)
...
with tab3:
    ...
    if not df_calc.empty:
        ...
        if st.button(f"Marcar Guia {n_pag} como Emitida para todos os clientes com PDF carregado"):
            ...
            st.session_state.clientes = df_full
            ...
            if not smtp_user or not smtp_pass:
                ...
                for i, nif in enumerate(selecionados):
                    ...
                    try:
                        ...
                        st.session_state.log_envio.append(
                ...
                st.session_state.clientes = df_full
                st.success(f"Concluído: {sucessos} enviados, {falhas} com erro.")
                st.rerun()
